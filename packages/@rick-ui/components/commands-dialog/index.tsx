  ...
  useEffect(() => {
    const fn = (e: KeyboardEvent) => {
      if ([Hotkey.ArrowUp, Hotkey.ArrowDown, Hotkey.Enter].includes(e.key)) {
        const item = document.querySelector<HTMLDivElement>(`.${commandItemCls}[data-type=item].${commandHoverCls}`);
        const items = document.querySelectorAll<HTMLDivElement>(`.${commandItemCls}[data-type=item]`);
        ...
        if (item) {
          ...
          const idx = Array.from(items).findIndex(i => i.classList.contains(commandHoverCls));
          ...
          onChange={e => {
            const result = commands.filter(i => {
              return (
                ...
                (i.mode === 'title' &&
                  commands.some(j => i.type === j.type && toLower(j.title).includes(toLower(e.target.value))))
              );
            });
          ...
          {displayCommandItems.map(item => {
            return (
              <div
                ...
                css={item.mode === 'title' ? CommandItemTitleStyle : CommandItemStyle}
                onClick={() => {
                  item?.onClick?.(item);
                }}>
                {item.title}
