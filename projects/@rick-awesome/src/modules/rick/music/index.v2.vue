    ...
    <div class="player">
      <div class="player__top">
        ...
        <div class="player-controls">
          <div
            ...
            :class="{ active: tracks[currentTrackIndex].favorites }"
            @click="favorite">
            <svg class="icon">
              <use xlink:href="#icon-heart-o"></use>
            </svg>
          </div>
          <a :href="github" target="_blank" class="player-controls__item">
            <svg class="icon">
      ...
      <div class="progress" ref="progress">
        <div class="progress__top">
          <div class="album-info" v-if="tracks[currentTrackIndex]">
...
<script lang="ts">
import { TrackType, tracks } from './config.ts';
  ...
  methods: {
    ...
    generateTime() {
      if (this.audio.duration) {
        ...
        this.duration = `${pad(durmin)}:${pad(dursec)}`;
        this.currentTime = `${pad(curmin)}:${pad(cursec)}`;
      }
    },

    async updateBar(offset: number) {
      const progress = document.querySelector<HTMLDivElement>('.progress')!;
      const maxduration = this.audio.duration;
      const position = offset - progress.getBoundingClientRect().left;
    ...
    favorite() {
      this.tracks[this.currentTrackIndex].favorites =
        !this.tracks[this.currentTrackIndex].favorites;
    },
  },
  created() {
...
.wrapper {
  ...
  min-height: calc(var(--layout-content-height) - 5vh);
